...
import {FileUploaderDropContainer, FileUploaderItem} from 'carbon-components-react';
...
const FileUploader = (props) => {
	...
	const handleWorker = async (file, message) => {
		...
		await prepareForUpload(name, extension, type, originalFileName, originalExtension, file, blob);
	}
	...
	const prepareForUpload = async (name, extension, type, originalFileName, originalFileExtension, file, blob) => {
		...
		showNotification({
			...
			subtitle: (
				<>
					...
					<p>Please allow a few minutes for the system to process it.</p>
				</>
			),
			timeout: 8000,
		});
	...
	return (
		<Container>
			...
			<p>Supported file types are .xlsx/.xlsm/.xlsb, .zip, .doc/.docx, .ppt/.pptx, .txt, .pdf, .csv, .print.</p>
		</Container>
	)
}
